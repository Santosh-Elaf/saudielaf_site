...
class Mig_Unyson_To_Codestar {
	...
	public function init() {
		...
		$fw_options = empty($options['fw_options']) ? [] : $options['fw_options'];
		...
		if( is_array($fw_options_child) && !empty( $fw_options_child ) ){
			foreach( $fw_options_child as $key => $value ){
				if(array_key_exists($key, $fw_options)){
					$fw_options[$key] = $value;
				}
			}
		}
